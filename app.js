...
const DEFAULT_DASHBOARD_ORDER = ["quest", "metrics", "top3", "activeProjects", "nextActions", "weeklyProgress", "pomodoro", "moodEnergy", "todayRoutines"];
...
async function whatShouldIDoNext() {
  await withAI("nextMoveBtn", async () => {
    ...
    const result = await callAgent("nextMove", { mood: log().mood, energy: log().energy, tasks: openTasks, routines: db.routines });
...
function dashboardWidgetHtml(widget, data) {
  ...
  const widgetMap = {
    ...
    todayRoutines: `<div class="card"><b>Today's Routines</b>${orderedItems("routines").map(routineItem).join("") || `<div class="empty">No routines yet.</div>`}</div>`
  };
...
function renderRoutines() {
  ...
  $("routines").innerHTML = `<div class="card between"><b>Routines</b><div class="row">${listSortControls('routines')}<button class="secondary small" onclick="expandAll('routine')">Expand All</button><button class="secondary small" onclick="collapseAll('routine')">Collapse All</button><button onclick="formRoutine()">Add</button><button class="secondary" onclick="suggestRoutine()">AI Suggest</button></div></div>${items.map(routineItem).join("") || `<div class="card empty">No routines yet.</div>`}`;
}
