  ...
  return `<div class="item ${(t.percent || 0) >= 100 ? "done" : ""}">
    <div class="between">
      <div style="flex:1">
        ...
        <div class="muted">${t.difficulty || "Easy"} · ${pointsFor(t.difficulty)} pts · ${t.percent || 0}%${subCount ? ` · ${subCount} subtasks` : ""}${t.due ? " · due " + t.due.replace("T", " ") : ""}</div>
  ...
  return `<div class="item ${done ? "done" : ""}">
    <div class="between">
      <div style="flex:1">
        ...
        <div class="muted">${r.time || ""} · ${r.recurrence || "Daily"}${subCount ? ` · ${subCount} steps` : ""}</div>
      </div>
...
function renderTasks() {
  $("tasks").innerHTML = `<div class="card between"><b>Tasks</b><div class="row"><button class="secondary small" onclick="expandAll('task')">Expand All</button><button class="secondary small" onclick="collapseAll('task')">Collapse All</button><button onclick="formTask()">Add Task</button></div></div>${db.tasks.map(itemTask).join("") || `<div class="card empty">No tasks yet.</div>`}`;
}
function renderRoutines() {
  $("routines").innerHTML = `<div class="card between"><b>Routines</b><div class="row"><button class="secondary small" onclick="expandAll('routine')">Expand All</button><button class="secondary small" onclick="collapseAll('routine')">Collapse All</button><button onclick="formRoutine()">Add</button><button class="secondary" onclick="suggestRoutine()">AI Suggest</button></div></div>${db.routines.sort((a,b)=>(a.time||"").localeCompare(b.time||"")).map(routineItem).join("") || `<div class="card empty">No routines yet.</div>`}`;
}
function renderProjects() {
  $("projects").innerHTML = `<div class="card between"><b>Projects</b><div class="row"><button class="secondary small" onclick="expandAll('project')">Expand All</button><button class="secondary small" onclick="collapseAll('project')">Collapse All</button><button onclick="formProject()">Add Project</button></div></div>` + 
  (db.projects.map(p => {
...
function renderGoals() {
  $("goals").innerHTML = `<div class="card between"><b>Goals</b><div class="row"><button class="secondary small" onclick="expandAll('goal')">Expand All</button><button class="secondary small" onclick="collapseAll('goal')">Collapse All</button><button onclick="formGoal()">Add Goal</button></div></div>` + 
  (db.goals.map(g => {
