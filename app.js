...
function routineRecurrenceOptions() {
  return ["Daily", "Weekdays", "Weekends", "Theme Day", "Custom Days"];
}
...
function formRoutine(r = {}) {
  ...
  modal(r.id ? "Edit Routine" : "Add Routine", `
    ...
    <label>Recurrence</label><select id="rRec" onchange="updateRoutineRecurrenceFields()">${routineRecurrenceOptions().map(x => `<option value="${x}" ${rec === x ? "selected" : ""}>${x}</option>`).join("")}</select>
    <div id="rThemeWrap">
