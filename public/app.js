...
function renderToday() {
  ...
  $("today").innerHTML = `
    <div class="card">
      <div class="between">
        <div><b>Today's Quest Points</b><strong style="font-size:2rem;display:block">${l.points}</strong><span class="muted">Goal: ${db.settings.dailyGoal} · AI uses today: ${l.aiUses || 0}/12</span></div>
    ...
    <div class="grid">
      ...
      <div class="metric"><span>🔥 Streak</span><strong>${db.settings.streak || 0}</strong><span class="muted">Minimum viable days count.</span></div>
    </div>
    ...
    <div class="card"><b>Due / Active Tasks</b>${db.tasks.filter(t => (t.percent || 0) < 100).slice(0, 7).map(itemTask).join("") || `<div class="empty">No active tasks yet. Tap + to add one.</div>`}</div>
